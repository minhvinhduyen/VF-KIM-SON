...
const App: React.FC = () => {
  ...
  const renderCarAnimation = () => (
    <div className="fixed inset-0 z-50 flex justify-center items-center bg-gradient-to-b from-sky-100/95 via-sky-50/95 to-gray-100/95 backdrop-blur-sm flex-col select-none overflow-hidden">
      {/* Clouds */}
  ...
  // Initial load before user logs in
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 font-sans relative">
      <Header />
      <main className="flex-grow p-2 md:p-6 relative">
      ...
      {/* Animation overlay: keeps current dashboard mounted in background without losing tab state */}
      {state.isLoading && renderCarAnimation()}
    </div>
  );
};
